...
touch "$REPO_ROOT/packages/adk-sim-protos/src/adk_sim_protos/__init__.py"

# Format Python generated code with ruff
echo "🎨 Formatting Python generated code..."
cd "$REPO_ROOT"
uv tool run ruff check --fix "$REPO_ROOT/packages/adk-sim-protos/src/adk_sim_protos" 2>/dev/null || true
uv tool run ruff format "$REPO_ROOT/packages/adk-sim-protos/src/adk_sim_protos"

# Feature Specification: Web UI for ADK Agent Simulator
...
### User Story 1 - Session Access and Navigation (Priority: P1)
...
### User Story 2 - Context Inspection via Event Stream (Priority: P1)
...
### User Story 3 - Tool Selection and Response Construction (Priority: P1)
...
### User Story 4 - Split-Pane Interface Layout (Priority: P2)
...
- ~~How does the system handle extremely large context windows?~~ *(Deferred: out of scope for this version)*
...
#### Context Inspection (Event Stream)
...
#### Response Construction (Control Panel)
...
- **Session**: A simulation session containing metadata (ID, creation time, status) and representing a connection between the ADK plugin and the web UI
- **LlmRequest**: The request data from the ADK plugin containing `contents` (history), `system_instruction`, `tools`, and `output_schema`
- **EventBlock**: A discrete unit in the Event Stream representing User Input, Agent Response, or Tool Execution
- **Tool**: A callable capability available to the agent, with name, description, and input schema
- **SmartBlob**: A string field that can be rendered in multiple formats (JSON tree, Markdown HTML, or raw text)
...
- ~~**SC-005**: System supports viewing context windows up to 100,000 tokens without significant performance degradation~~ *(Deferred: out of scope for this version)*

...
"""
  ...
  job_config = bigquery.QueryJobConfig(
    query_parameters=[
      bigquery.ArrayQueryParameter("timeseries_codes", "STRING", timeseries_codes),
...
def number_of_moves(front_ff_future_px: float, back_ff_future_px: float) -> float:
  ...
  return rate_delta / move_size_pct
...
def compute_probabilities(meeting_date_str: str) -> dict:
  ...
  for code in timeseries_codes:
    if code not in prices:
      ...
      break
    if meeting_date not in prices[code]:
      error = f"No data for {code} on {meeting_date}"
      break
    if meeting_date_day_before not in prices[code]:
  ...
  output = {
    "Odds of a rate move within the next year (computed before Fed meeting)": (
      probs_pre
    ),
    "Odds of a rate move within the next year (computed after Fed meeting)": (
      probs_post
    ),
  }
